...
class NormProduct(nn.Linear):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.k == 1:
            ...
            cosine_all = cosine_all.view(-1, self.out_features, self.k) # (N, cls_num, k_centers)
            cosine, _ = torch.max(cosine_all, dim=2) # 选出每个样本最大的sub-center属于哪个cls
            return cosine
...
class ArcFaceClsHead(ClsHead):
    ...
    def __init__(self,
        ...
        self.register_buffer( # register_buffer的参数可以直接被 self.margins 这种形式修改和调用
            'margins', torch.tensor(margins).float(), persistent=False) # 把每个类别的margin注册成buffer
        # To make `phi` monotonic decreasing, refers to
        # https://github.com/deepinsight/insightface/issues/108
        sinm_m = torch.sin(math.pi - self.margins) * self.margins # sin(pi - m) * m = sin(m) * m
        threshold = torch.cos(math.pi - self.margins) # cos(pi - m) = -cos(m)
        self.register_buffer('sinm_m', sinm_m, persistent=False)
        self.register_buffer('threshold', threshold, persistent=False)

    def set_margins(self, margins: Union[Sequence[float], float]) -> None: # 这个给 adaptive margin hook 调用
    ...
    def _get_logit_with_margin(self, pre_logits, target):
        ...
        phi = torch.cos(torch.acos(cosine) + self.margins) # phi所有的 cosine 结果都加上margin

        if self.easy_margin:
            # when cosine>0, choose phi
            # when cosine<=0, choose cosine, easy_margin下需要保证 cosine > 0，即为锐角，才会加上margin
            phi = torch.where(cosine > 0, phi, cosine)
        else:
            # when cos>th, choose phi
            # when cos<=th, choose cosine-mm, 非easy_margin下，需要cosine>cos(pi-m)，即保证 theta + m 小于 pi，才能保证正的优化方向，
            # 否则不直接从角度调整margin，直接加上 m * sin(m) 这个 buffer 作为margin
            phi = torch.where(cosine > self.threshold, phi,
                              cosine - self.sinm_m)

        target = convert_to_one_hot(target, self.num_classes)
        output = target * phi + (1 - target) * cosine # target对应的cosine logit保留margin，其他位置全部替换成原始的cosine
        return output
