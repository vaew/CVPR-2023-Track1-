...
class BaseTTAModel(BaseModel):
    ...
    @abstractmethod
    def det_merge_preds(self, data_samples_list: EnhancedBatchDataSamples) \
            -> MergedDataSamples:
        """Merge predictions of enhanced data to one prediction.
    ...
    @abstractmethod
    def seg_merge_preds(self, data_samples_list: EnhancedBatchDataSamples) \
            -> MergedDataSamples:
        """Merge predictions of enhanced data to one prediction.
    ...
    @abstractmethod
    def cls_merge_preds(self, data_samples_list: EnhancedBatchDataSamples) \
            -> MergedDataSamples:
        """Merge predictions of enhanced data to one prediction.
    ...
    def test_step(self, data):
        ...
        if isinstance(data, dict) and 'det' in data.keys():
            det_data=data['det']
            num_augs = len(det_data[next(iter(det_data))])
            data_list = [{'det':{key: value[idx]
                          for key, value in det_data.items()}}
                         for idx in range(num_augs)]
            
            predictions = []
            for data in data_list:  # type: ignore
                prediction = self.module.test_step(data)
                predictions.append(prediction['det'])
            return self.det_merge_preds(list(zip(*predictions)))  # type: ignore

        elif isinstance(data, dict) and 'seg' in data.keys():
            det_data=data['seg']
            num_augs = len(det_data[next(iter(det_data))])
            data_list = [{'seg':{key: value[idx]
                          for key, value in det_data.items()}}
                         for idx in range(num_augs)]
            ...
            for data in data_list:  # type: ignore
                prediction = self.module.test_step(data)
                predictions.append(prediction['seg'])
            return self.seg_merge_preds(list(zip(*predictions)))  # type: ignore

        elif isinstance(data, dict) and 'cls' in data.keys():
            det_data=data['cls']
            num_augs = len(det_data[next(iter(det_data))])
            data_list = [{'cls':{key: value[idx]
                          for key, value in det_data.items()}}
                         for idx in range(num_augs)]
            ...
            for data in data_list:  # type: ignore
                prediction = self.module.test_step(data)
                predictions.append(prediction['cls'])
            return self.cls_merge_preds(list(zip(*predictions)))  # type: ignore
