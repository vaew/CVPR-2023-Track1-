...
from mmengine.dataset import MultiTaskDataLoader, MultiTaskDataLoaderVal # by ZeLun Zhang
...
class Runner:
    ...
    Args:
        ...
        train_dataloader (Dataloader or dict, optional): A dataloader object or
            ...
            skipping training steps. Defaults to None. 如果None则跳过train
            See :meth:`build_dataloader` for more details.
        val_dataloader (Dataloader or dict, optional): A dataloader object or
            a dict to build a dataloader. If ``None`` is given, it means
            skipping validation steps. Defaults to None. 如果None则跳过test
            See :meth:`build_dataloader` for more details.
    ...
    def __init__(
        ...
        self._check_scheduler_cfg(param_scheduler) # 如果单个optim, 是list，如果多个optim，是dict
        ...
        # See `MessageHub` and `ManagerMixin` for more details. 主要用来获得一些和训练相关的常量信息
    ...
    def build_optim_wrapper(
        ...
        It will build a :obj:`OptimWrapper` by default. # 如果只有一个optimizer，不申命type时会默认用OptimWrapper
        ...
        training with multiple optimizers. # 如果 cfg 中的optim_wrapper有多个optim_wrapper，则需要显示设定constructor
    ...
    def build_evaluator(self, evaluator: Union[Dict, List,Evaluator]) -> Evaluator:
        ...
        if isinstance(evaluator, Evaluator):
            ...
            if evaluator.get('type', None) is not None: # 单任务
                return self.build_single_task_evaluator(evaluator)
            else: # 多任务
                task2evaluator = {}
                for task in evaluator:
                    task2evaluator[task] = self.build_single_task_evaluator(evaluator[task])
                return task2evaluator
        else:
            raise NotImplementedError

    def build_single_task_evaluator(self, evaluator: Union[Dict, List, # 构建单任务的evaluator
                                               Evaluator]) -> Evaluator:
        """Build evaluator.
    ...
    def build_dataloader(dataloader: Union[DataLoader, Dict],
        ...
        '''by ZeLun Zhang, 支持建立多任务的dataloader
        '''
        if isinstance(dataloader, DataLoader):
            return dataloader

        if dataloader.get('dataset', None) is not None: # 单任务
            return Runner.build_single_task_dataloader(dataloader, seed, diff_rank_seed)
        else: # 多任务
            if dataloader.mode == 'train': # 同时从多任务中加载数据
                task2dataloader = {}
                for task, dataloader_cfg in dataloader.items():
                    if task == 'mode':
                        continue
                    task2dataloader[task] = Runner.build_single_task_dataloader(dataloader_cfg, seed, diff_rank_seed)
                return MultiTaskDataLoader(task2dataloader)
            elif dataloader.mode == 'val': # 按照顺序从dataloader
                task2dataloader = {}
                for task, dataloader_cfg in dataloader.items():
                    if task == 'mode':
                        continue
                    task2dataloader[task] = Runner.build_single_task_dataloader(dataloader_cfg, seed, diff_rank_seed)
                return MultiTaskDataLoaderVal(task2dataloader)
            else:
                raise NotImplementedError(f"mode: {dataloader.mode} not implemented.")

    @staticmethod
    def build_single_task_dataloader(dataloader: Union[DataLoader, Dict], # 原 build_dataloader
                         seed: Optional[int] = None,
                         diff_rank_seed: bool = False) -> DataLoader:
        """Build dataloader.
    ...
    def train(self) -> nn.Module:
        ...
        assert hasattr(ori_model, 'train_step'), ( # train_cfg构建train_loop, 调用模型的train_step
            'If you want to train your model, please make sure your model '
            'has implemented `train_step`.')

        if self._val_loop is not None:
            assert hasattr(ori_model, 'val_step'), ( # val_cfg构建val_loop, 调用模型的val_step
    ...
    def resume(self,
               ...
               map_location: Union[str, Callable] = 'cpu') -> None:
