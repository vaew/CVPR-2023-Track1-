...
from mmengine.dataset import MultiTaskDataLoader
...
class EpochBasedTrainLoop(BaseLoop):
    ...
    def run(self) -> torch.nn.Module: # runner 的 train 中调用这个函数
        """Launch training."""
        self.runner.call_hook('before_train'); #print('before_train')

        while self._epoch < self._max_epochs and not self.stop_training:
            self.run_epoch()

            self._decide_current_val_interval()
            #print(f'{self.__class__.__name__}.val_interval = {self.val_interval}')
            #print(f'self._epoch = {self._epoch}, self.val_begin = {self.val_begin}, self._epoch = {self._epoch}')
        ...
        self.runner.call_hook('after_train'); #print('after train')
        return self.runner.model

    def run_epoch(self) -> None:
        """Iterate one epoch."""
        self.runner.call_hook('before_train_epoch'); #print('before_train_epoch')
        self.runner.model.train()
        #print(self.dataloader.dataset, len(self.dataloader.dataset))
        for idx, data_batch in enumerate(self.dataloader):
            self.run_iter(idx, data_batch)

        self.runner.call_hook('after_train_epoch'); #print('after_train_epoch')
        self._epoch += 1
...
class IterBasedTrainLoop(BaseLoop):
    ...
    def __init__(
        ...
        if isinstance(self.dataloader, MultiTaskDataLoader):
            self.multi_task_flag = True

        if not self.multi_task_flag:
            if hasattr(self.dataloader.dataset, 'metainfo'):
                self.runner.visualizer.dataset_meta = \
                    self.dataloader.dataset.metainfo
            else:
            ...
            self.runner.visualizer.dataset_meta = self.dataloader.src_task2dataloader['det'].dataset.metainfo
    ...
    def run(self) -> None:
        ...
        while self._iter < self._max_iters and not self.stop_training:
            ...
            #print(data_batch.keys()) # 没问题，加载三个任务的数据
    ...
    def run_iter(self, data_batch: Sequence[dict]) -> None:
        ...
        # synchronization during gradient accumulation process.
        # outputs should be a dict of loss.
        # print(self.runner.model.__class__.__name__) # 竟然是 MMDDP
        outputs = self.runner.model.train_step(
            data_batch, optim_wrapper=self.runner.optim_wrapper)
...
class ValLoop(BaseLoop):
    ...
    def __init__(self,
        ...
        if isinstance(evaluator, dict) or isinstance(evaluator, list):
            self.evaluator = runner.build_evaluator(evaluator)  # 多任务返回dict, key为 {'det', 'seg', 'cls'}
        ...
        if not self.multi_task_flag:
            ...
            for task in self.evaluator:
                self.evaluator[task].dataset_meta = self.dataloader.src_task2dataloader[task].dataset.metainfo
                self.runner.visualizer.dataset_meta = \
                    self.dataloader.src_task2dataloader[task].dataset.metainfo
        self.fp16 = fp16

    def run(self) -> dict:
        """Launch validation."""
        #print('in val loop run')
        self.runner.call_hook('before_val')
        self.runner.call_hook('before_val_epoch')
        self.runner.model.eval()
        for idx, data_batch in enumerate(self.dataloader): # 多任务时，依次从各个任务的dataloader加载数据
            self.run_iter(idx, data_batch)

        # compute metrics
        if not self.multi_task_flag:
            metrics = self.evaluator.evaluate(len(self.dataloader.dataset))
        else: # 多任务下对每个任务分别计算metrics
            metrics_dict = {}
            for task in self.evaluator:
                metrics_dict[task] = self.evaluator[task].evaluate(len(self.dataloader.src_task2dataloader[task].dataset))
            # 存储了多任务的预测结果
            metrics = {}
            for task in metrics_dict:
                task_metric_dict = metrics_dict[task]
                for k in task_metric_dict:
                    metrics[f'{task}_' + k] = task_metric_dict[k]
    ...
    @torch.no_grad()
    def run_iter(self, idx, data_batch: Sequence[dict]): # 直接从val_dataloader中加载的数据
        """Iterate one mini-batch.
        ...
        if not self.multi_task_flag:
            self.evaluator.process(data_samples=outputs, data_batch=data_batch) # 每次val_batch结束调用process
        else:
            for task in data_batch: # 多任务则调用各自的evaluator
                self.evaluator[task].process(data_samples=outputs[task], data_batch=data_batch[task])
