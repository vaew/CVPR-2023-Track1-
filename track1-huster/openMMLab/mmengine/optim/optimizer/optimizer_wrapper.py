...
class OptimWrapper:
    ...
    def _clip_grad(self, task = None, backbone_based = False, model:nn.Module = None) -> None:
        """Clip the gradients of parameters."""
        '''task: 任务名，方便message_hub统计不同任务的梯度
           backbone_based: 是否基于backbone修剪梯度
           model: 如果基于backbone修剪梯度，需要传入model
        '''
        if not backbone_based:
            params: List[torch.Tensor] = []
            for param_group in self.optimizer.param_groups:
                params.extend(param_group['params'])

            params = list(
                filter(lambda p: p.requires_grad and p.grad is not None, params)) # 过滤，找出param中requires_grad并且有grad的参数
            if len(params) > 0:
                ...
                if grad is not None:
                    if task is None:
                        ...
                        self.message_hub.update_scalar(f'train/{task}-{self.grad_name}',
                                                        float(grad))
        else:
            assert model is not None, f"model can't be None when clip grad is backbone based."
            params: List[torch.Tensor] = []
            for name, param in model.module.named_parameters():
                if 'backbone' in name:
                    params.append(param)
            params = list(filter(lambda p: p.requires_grad and (p.grad is not None), params))
            # 以上获得所有 backbone 中的 params
            norm_type = self.clip_grad_kwargs['norm_type']
            max_norm = self.clip_grad_kwargs['max_norm']
            if len(params) > 0:
                # 当前backbone的grad_norm
                device = params[0].grad.device
                backbone_grad_norm = torch.norm(torch.stack([torch.norm(p.grad.detach(), norm_type).to(device) for p in params]), norm_type)
                clip_coef = max_norm / (backbone_grad_norm + 1e-6)
                clip_coef_clamped = torch.clamp(clip_coef, max=1.0)
                for p in model.module.parameters(): # 对于所有的参数都要梯度修剪
                    if p.grad is None:
                        continue
                ...
                if backbone_grad_norm is not None:
                    if task is None:
                        self.message_hub.update_scalar(f'train/{self.grad_name}',
                                                        float(backbone_grad_norm))
                    else:
                        self.message_hub.update_scalar(f'train/{task}-backbone-{self.grad_name}',
                                                        float(backbone_grad_norm))
