...
class MultiImageMixDataset:
    ...
    def __getitem__(self, idx):
        ...
        for (transform, transform_type) in zip(self.pipeline,
            ...
            if transform.__class__.__name__ == 'RandomChoice' or transform.__class__.__name__ == 'RandomChoiceSegTrack1':
                for compose_transforms in transform.transforms:
                    assert compose_transforms.__class__.__name__ == 'Compose', f'{compose_transforms.__class__.__name__}'
                    for t in compose_transforms.transforms:
                        if t.__class__.__name__ == 'RandomMosaic':
                            indices = t.get_indices(self.dataset)
                            if not isinstance(indices, collections.abc.Sequence):
                                indices = [indices]
                            mix_results = [
                                copy.deepcopy(self.dataset[index]) for index in indices
