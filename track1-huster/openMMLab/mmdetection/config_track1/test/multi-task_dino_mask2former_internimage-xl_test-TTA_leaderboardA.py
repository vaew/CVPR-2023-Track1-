...
det_img_scales = [(832, 832), (1280, 1024), (1536, 1536), (2048, 2048)]
...
det_train_mosaic_pipeline = [
    dict(type='Mosaic', img_scale=(1024, 1024), pad_val=114.0), # 2048x2048
    dict(
        type='RandomAffine',
        max_rotate_degree = 0,
        max_shear_degree = 0,
        ...
        border=(-1024 // 2, -1024 // 2), # 注意是负数, 输出 1024x1024
    ),
    dict( 
        type='MixUp', # 需要增加概率
        img_scale=(1024,1024), # 从dataset中采样图像的scale
        ratio_range=(0.5, 1.5), 
        prob = 1.0,
        pad_val=114.0,
    ),
        ...
        transforms=[
            [ # 单纯对每张图片多尺度训练，对图像整体的 keep_ratio 的缩放, 本质在提高小目标的鲁棒性
                dict(
                    ...
                    scales=[(800, 1024), (832, 1024), (864, 1024), (896, 1024), (928, 1024), (960, 1024), (992, 1024),
                            (1024, 1024)], # 最小分辨率800x800, 最大分辨率1024x1024, 不用管数据集中原图的尺寸，对于模型来说只有测试时的尺寸
            ...
            [
                dict( # 这里是在增加大目标的鲁棒性
                    ...
                    scales=[(832, 1024), (928, 1024), (1024, 1024)], # 先随机整体缩放到 832, 928, 1024,
                    keep_ratio=True
                ),
                dict(
                    type='RandomCrop',
                ...
                dict(
                    type='RandomChoiceResize',
                    scales=[(800, 1024), (832, 1024), (864, 1024), (896, 1024), (928, 1024), (960, 1024), (992, 1024),
                            (1024, 1024)], # 最小分辨率800x800, 最大分辨率1024x1024
                    keep_ratio=True
                )
            ]
        ]
...
det_test_pipeline = [
    ...
    dict(type='Resize', scale=(1024, 1024), keep_ratio=True), # 测试时就是 1024x1024
...
det_val_evaluator = dict(
    ...
    format_only=False,
    backend_args=backend_args,
    #outfile_prefix = '../work_dirs/multi-task_dino_segformer_internimage-xl_imgnet-pretrain_st1', # 保存提交结果
    )
det_test_evaluator = det_val_evaluator

# seg的metric
seg_val_evaluator = dict(
    type='mmseg.IoUMetric', 
    iou_metrics=['mIoU', 'mFscore'],
    #output_dir='../work_dirs/submit_seg', # 保存提交结果
)
seg_test_evaluator = seg_val_evaluator

# cls的metric
cls_val_evaluator = dict(
    type='mmpretrain.Accuracy', 
    topk=(1, 5),
    #results_saveroot = '../work_dirs/submit' # 保存提交结果
    )
...
# 训练逻辑 loop 设置
train_cfg = dict(
    type='IterBasedTrainLoop', max_iters=35200, val_interval=440) # 450
# 测试 loop 设置
...
default_hooks = dict(
    ...
    checkpoint=dict(type='CheckpointHook', by_epoch=False, interval=440), # 450
