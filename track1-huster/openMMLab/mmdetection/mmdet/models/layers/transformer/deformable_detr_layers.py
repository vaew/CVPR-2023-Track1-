...
class DeformableDetrTransformerEncoder(DetrTransformerEncoder):
    ...
    def get_encoder_reference_points(
        ...
        for lvl, (H, W) in enumerate(spatial_shapes): # 每个level特征的形状, 对pad后image由backbone提取的特征
            ...
            # ref_y: [H, W], ref_x: [H, W], 根据H,W生成每个 level特征的 ref point，是每个像素的中心
            # [1, H*W] / [bs, 1] -> [bs, H*W], [(valid_ratios[:, None, lvl, 1] * H) 计算当前特征图是原图像范围的长度]
            ref_y = ref_y.reshape(-1)[None] / (valid_ratios[:, None, lvl, 1] * H)
            ref_x = ref_x.reshape(-1)[None] / (valid_ratios[:, None, lvl, 0] * W)
            ref = torch.stack((ref_x, ref_y), -1) # [bs, H*W, 2]
            reference_points_list.append(ref)
        reference_points = torch.cat(reference_points_list, 1) # [bs, sum(level_hw), 2]
        # reference_points[:, :, None] -> [bs, sum(level_hw), 1, 2], valid_ratios[:, None] -> [bs, 1, num_levels, 2]
        # [bs, sum(level_HW), num_levels, 2], 每个level上每个像素点，在别的level上的想reference点，注意，其都是相对pad后的特征图
    ...
    def forward(self,
        ...
        for layer_id, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 4:
                ...
                assert reference_points.shape[-1] == 2 # two_stage = False, 原始输入的reference_ptns仅仅是一个点
                reference_points_input = \
                    reference_points[:, :, None] * \
                    valid_ratios[:, None] # [bs, num_queries, 1, 2] * [bs, 1, num_levels, 2], 把每个query坐标都限定在图像的原始区域范围内
                # 输入的reference_points经过sigmoid值在[0, 1]之间，但是这是对于pad后图像的，需要把其值缩放到原始图像范围内
            output = layer(
                output, # 上一层输出的query
                query_pos=query_pos, # decoder中nn.embedding的posi encoding
                value=value, # 一直就是encoder输出的memory
                key_padding_mask=key_padding_mask,
                spatial_shapes=spatial_shapes,
                level_start_index=level_start_index,
                valid_ratios=valid_ratios,
                reference_points=reference_points_input, # 一直就是 decoder最开始通过 fc + sigmoid 推理nn.embedding获得的坐标
                **kwargs)
