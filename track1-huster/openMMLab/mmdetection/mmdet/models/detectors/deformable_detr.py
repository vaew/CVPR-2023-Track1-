...
class DeformableDETR(DetectionTransformer):
    ...
    def _init_layers(self) -> None:
        ...
        self.positional_encoding = SinePositionalEncoding( # 初始化 encoder 加到输入的encoding
        ...
        self.level_embed = nn.Parameter( # backbone每个level的特征，标志这个特征来自哪个level
    ...
    def pre_transformer(
        ...
        # construct binary masks for the transformer. detector训练会使用过padding，每个batch内所有图片都padding到相同尺寸
        assert batch_data_samples is not None
        batch_input_shape = batch_data_samples[0].batch_input_shape # 经过padding后整个batch的h,w
        img_shape_list = [sample.img_shape for sample in batch_data_samples] # 每个sample的实际形状
        input_img_h, input_img_w = batch_input_shape
        masks = mlvl_feats[0].new_ones((batch_size, input_img_h, input_img_w)) # [B, pad后的H, pad后的W]
        for img_id in range(batch_size):
            img_h, img_w = img_shape_list[img_id]
            masks[img_id, :img_h, :img_w] = 0 # 图像的实际内容集中在左上角，mask指定图像的原始内存在padding后的tensor的哪个位置
        # NOTE following the official DETR repo, non-zero values representing
        # ignored positions, while zero values means valid positions.

        mlvl_masks = [] # backbone输出的每个level的feature中实际是 原始图像内容的mask
        mlvl_pos_embeds = [] # 加到 backbone输出的每个level特征 的sin-cos encoding, 形状 [B, layer_h, layer_w, c]
        for feat in mlvl_feats:
            mlvl_masks.append(
                F.interpolate(masks[None], size=feat.shape[-2:]).to(torch.bool).squeeze(0) # nearest插值
                )
            mlvl_pos_embeds.append(self.positional_encoding(mlvl_masks[-1]))
        # 以上生成 *原始图像在backbone输出的每层特征对应的mask*, *加到backbone每层特征的cos-sin encoding*

        feat_flatten = [] # backbone 每个level的特征，[B, layer_h * layer_w, c]
        lvl_pos_embed_flatten = [] # 每个level的encoding, 即 pos_encoding + layer_encoding, 形状
        mask_flatten = [] # 每个level特征的mask，标志原始图像内容位置的mask, [bs, h_lvl*w_lvl]
        spatial_shapes = [] # 每个level特征的形状
        for lvl, (feat, mask, pos_embed) in enumerate(
            ...
            lvl_pos_embed = pos_embed + self.level_embed[lvl].view(1, 1, -1) # 取出 level_embedding 和特征相加
        ...
        # feat_flatten: List[tensor[B, layer_h * layer_w, c], ...], Batch中每个level的特征
        # lvl_pos_embed_flatten:  形状同上，Batch中每个level的 pos + layer 的encoding
        # mask_flatten: List[tensor[B, layer_h * layer_w]], 每张图片在每个level中对应的原始图片内容的mask

        # (bs, num_feat_points, dim)
        feat_flatten = torch.cat(feat_flatten, 1) # Batch中每张图片所有level的特征的concat
        lvl_pos_embed_flatten = torch.cat(lvl_pos_embed_flatten, 1)
        # (bs, num_feat_points), where num_feat_points = sum_lvl(h_lvl*w_lvl)
        mask_flatten = torch.cat(mask_flatten, 1) # Batch中每张图片所有level的特征的对应的原图mask
        # 以上准备好transformer的输入
        
        spatial_shapes = torch.as_tensor(  # (num_level, 2), 每个level对应的特征形状(padding后)
            spatial_shapes,
            dtype=torch.long,
            device=feat_flatten.device
            )
        level_start_index = torch.cat(( # (num_level), feat_flatten每个元素(bs, num_feat_points, dim), num_feat_points是所有点cat, 记录每个level的start_idx
            spatial_shapes.new_zeros((1, )), 
            spatial_shapes.prod(1).cumsum(0)[:-1]) # prod(1)沿着维度1求乘积 之后 累加
            )
        valid_ratios = torch.stack(  # (bs, num_level, 2), 每张图片原始图片内容占据pad后图像的H, w的ratio
            [self.get_valid_ratio(m) for m in mlvl_masks], 1
            )

        encoder_inputs_dict = dict(
            feat=feat_flatten, # (bs, num_feat_points, dim)
            feat_mask=mask_flatten, # (bs, num_feat_points)
            feat_pos=lvl_pos_embed_flatten, # (bs, num_feat_points, dim)
            spatial_shapes=spatial_shapes, # (num_level, 2)
            level_start_index=level_start_index, # (num_level)
            valid_ratios=valid_ratios) # (bs, num_level, 2)
        decoder_inputs_dict = dict(
            memory_mask=mask_flatten, # (bs, num_feat_points)
            spatial_shapes=spatial_shapes, # (num_level, 2)
            level_start_index=level_start_index, # (num_level)
            valid_ratios=valid_ratios) # (bs, num_level, 2)
    ...
    def pre_decoder(self, memory: Tensor, memory_mask: Tensor,
        ...
        if self.as_two_stage:
            ...
            query = query.unsqueeze(0).expand(batch_size, -1, -1) # decoder的query就是300个 nn.embedding
            reference_points = self.reference_points_fc(query_pos).sigmoid() # [bs, num_queries, 2], 每张图片的结果都相等。
            # 整个decoder原始输入的reference_point, 输入的query直接来自nn.embedding，对应的reference_pnts由一个fc在另外一个nn.embedding获得

        decoder_inputs_dict = dict(
            query=query, # query来自nn.embedding
            query_pos=query_pos, # 输入的query_pos来自nn.embeding
            memory=memory,
            reference_points=reference_points # reference_point来自一个fc对query_pos的预测再加上sigmoid
            )
