...
class ClassAwareSampler(Sampler):
    ...
    def __init__(self,
        ...
        # get number of images containing each category, 每个类别下图片的数量
    ...
    def __iter__(self) -> Iterator[int]: # 首先基于类别采样，然后再在类别下的图像中采样，从而保证 class-balanced
        # deterministically shuffle based on epoch
        g = torch.Generator()
        g.manual_seed(self.epoch + self.seed)

        # initialize label list, 每个类别的随机采样
        label_iter_list = RandomCycleIter(self.valid_cat_inds, generator=g)
        # initialize each per-label image list
        data_iter_dict = dict()
        for i in self.valid_cat_inds: # 每个类别下每张图片的随机采样
        ...
        def gen_cat_img_inds(cls_list, data_dict, num_sample_cls):
            ...
            id_indices = []
            for _ in range(len(cls_list)): # 每个类别循环
                cls_idx = next(cls_list)
                for _ in range(num_sample_cls): # 每个类别单次采样数量循环
                    id = next(data_dict[cls_idx])
                    id_indices.append(id)
            return id_indices # 最终采样出的图像索引

        # deterministically shuffle based on epoch
        num_bins = int( # 总共的采样次数 = 一次ep的样本数量 / 类别数量 / 每次采样每个类别多少样本, 每个类别在一次ep中采样 num_bins * self.num_sample_class 个
        ...
        return iter(indices) # 确定了整个epoch中使用的样本索引
