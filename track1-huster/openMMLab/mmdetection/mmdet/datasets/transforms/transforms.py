...
class RandomCrop(BaseTransform):
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        if results.get('gt_bboxes', None) is not None:
            ...
            bboxes.translate_([-offset_w, -offset_h]) # 调整bbox相对位置
...
class Mosaic(BaseTransform):
    ...
    Args:
        img_scale (Sequence[int]): Image size after mosaic pipeline of single
            ...
            Defaults to (640, 640). mosic后单张图像的尺寸，mosic是2x2拼接，所以最后尺寸会翻两倍。
    ...
    def transform(self, results: dict) -> dict:
        ...
        Returns:
            dict: Updated result dict.
        """
        mix_results_name = 'mosaic_mix_results'
        if random.uniform(0, 1) > self.prob:
            return results

        assert mix_results_name in results
        mosaic_bboxes = []
        mosaic_bboxes_labels = []
        mosaic_ignore_flags = []
        if len(results['img'].shape) == 3:
            mosaic_img = np.full( # 输出图像的尺寸，mosic拼接2x2，最后输出2倍
        ...
        center_position = (center_x, center_y) # 在mosaic_img中采样center，在[0.5, 1.5]之间
        ...
        for i, loc in enumerate(loc_strs):
            if loc == 'top_left':
                ...
                results_patch = copy.deepcopy(results[mix_results_name][i - 1])

            img_i = results_patch['img']
            h_i, w_i = img_i.shape[:2]
            # keep_ratio resize，做rescale，缩放到 self.img_scale 内
            ...
            x1_p, y1_p, x2_p, y2_p = paste_coord # 在 mosaic_img 中的粘贴位置
            x1_c, y1_c, x2_c, y2_c = crop_coord # 在 patch_img 中的 crop 位置
            ...
            # 只有左上角的pad会对bbox位置产生影响, paste_coord - crop_coord 直接获得相对于 paste_img 左上角的pad
            padw = x1_p - x1_c
            padh = y1_p - y1_c
            gt_bboxes_i.rescale_([scale_ratio_i, scale_ratio_i])
            gt_bboxes_i.translate_([padw, padh]) # 经过translate后部分bbox超出mosaic_img的范围
        ...
        # remove outside bboxes, 确实把超出mosaic_img的部分的bbox移除了
    ...
    def _mosaic_combine(
        ...
        Returns:
            tuple[tuple[float]]: Corresponding coordinate of pasting and
                cropping
                - paste_coord (tuple): paste corner coordinate in mosaic image. 在最后 mosaic_img 中粘贴这个patch的位置
                - crop_coord (tuple): crop corner coordinate in mosaic image.
        """
        assert loc in ('top_left', 'top_right', 'bottom_left', 'bottom_right')
        if loc == 'top_left':
            # index0 to top left part of image
            # 确定paste_coord，patch_img小于top-left预留位置，才会有pad
            x1, y1, x2, y2 = max(center_position_xy[0] - img_shape_wh[0], 0), \
                             max(center_position_xy[1] - img_shape_wh[1], 0), \
                             center_position_xy[0], \
                             center_position_xy[1]
            # 确定patch_img中的crop位置，剪裁图像的右下角
...
class MixUp(BaseTransform):
    ...
    def __init__(self,
                 img_scale: Tuple[int, int] = (640, 640), # 并不是最终输出尺寸，最终输出尺寸为results['img']的尺寸
    ...
    def transform(self, results: dict) -> dict:
        ...
        Returns:
            dict: Updated result dict.
        """
        mix_results_name = 'mixup_mix_results'
        if random.random() > self.prob:
            if mix_results_name in results:
                del results[mix_results_name]
            return results

        assert mix_results_name in results
        assert len(
            results[mix_results_name]) == 1, 'MixUp only support 2 images now !'

        if results[mix_results_name][0]['gt_bboxes'].shape[0] == 0:
            # empty bbox, 如果经过原始 dataset pipeline 后的图像无gt，则不做mixup
            return results

        retrieve_results = results[mix_results_name][0]
        retrieve_img = retrieve_results['img']

        jit_factor = random.uniform(*self.ratio_range) # 是对scale的jitter
        is_filp = random.uniform(0, 1) > self.flip_ratio

        if len(retrieve_img.shape) == 3:
            out_img = np.ones( # mixup最终结果
        ...
        # 1. keep_ratio resize, 对图像rescale，不超过img_scale的范围, 是从dataset中抽样的图像
        ...
        # 2. paste, 从数据集中抽样的图像粘贴在左上角
        out_img[:retrieve_img.shape[0], :retrieve_img.shape[1]] = retrieve_img

        # 3. scale jit, jit_factor是对scale的jitter，对out_img做rescale, out_img有multi_scale
        ...
        target_h, target_w = ori_img.shape[:2] # result['img']尺寸作为target_hw
